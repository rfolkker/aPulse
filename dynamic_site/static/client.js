...
document.addEventListener("DOMContentLoaded", async () => {
	...
	const refreshStatus = async () => {
		try {
			// get the instance name from the instance_id dropdown list, or default to default-instance if not found
			const instanceId = document.querySelector('#instance_id')?.value || 'default-instance';
			const response = await fetch(`./status?instanceId=${instanceId}`, {cache: "no-cache"});
